# 🔥 GeeksforGeeks 60-Day Streak Challenge

> My journey of solving one problem every day for 60 consecutive days

[![Streak](https://img.shields.io/badge/Current%20Streak-5%2F60-brightgreen)]()
[![Language](https://img.shields.io/badge/Language-Java-orange)]()
[![Challenge](https://img.shields.io/badge/Challenge-GFG%202026-blue)]()

## 📌 About This Repository

This repository documents my participation in the **GeeksforGeeks 60-Day Streak Challenge (2026)**, where I commit to solving one Problem of the Day (POTD) for 60 consecutive days.

**Challenge Duration:** Feb 13, 2026 - Apr 13, 2026

**Goal:** Build consistency, strengthen DSA fundamentals, and maintain a public accountability system.

---

## 🎯 Challenge Rules

✅ Solve one GFG Problem of the Day (POTD) daily  
✅ Maintain a 60-day streak without breaks  
✅ Document solutions with clear explanations  
✅ Track progress publicly

---

## 📊 Progress Tracker

| Week | Days Completed | Status |
|------|----------------|--------|
| Week 1 (Feb 13-19) | 5/7 | 🟢 In Progress |
| Week 2 (Feb 20-26) | 0/7 | ⏳ Upcoming |
| Week 3 (Feb 27-Mar 5) | 0/7 | ⏳ Upcoming |
| Week 4 (Mar 6-12) | 0/7 | ⏳ Upcoming |
| Week 5 (Mar 13-19) | 0/7 | ⏳ Upcoming |
| Week 6 (Mar 20-26) | 0/7 | ⏳ Upcoming |
| Week 7 (Mar 27-Apr 2) | 0/7 | ⏳ Upcoming |
| Week 8 (Apr 3-9) | 0/7 | ⏳ Upcoming |
| Week 9 (Apr 10-13) | 0/4 | ⏳ Upcoming |

**Total Progress: 5/60 Days (8.33%)**

---

## 📁 Repository Structure

```
GFG_daily_practice_problem/
│
├── README.md
├── Week1/
│   ├── Day01_Painters_Partition.java
│   ├── Day02_Problem_Name.java
│   └── ...
├── Week2/
│   └── ...
└── Solutions_Index.md
```

---

## 🗂️ Solutions Index

### Week 1 (Feb 13-19, 2026)

| Day | Date | Problem | Difficulty | Topics | Solution |
|-----|------|---------|------------|--------|----------|
| 1 | Feb 13 | [The Painter's Partition Problem-II](https://www.geeksforgeeks.org/problems/the-painters-partition-problem1535/1) | Hard 🔥 | Binary Search, Greedy | [View](./Week1/Day01_Painters_Partition.java) |
| 2 | Feb 14 | [Chocolate Distribution Problem](https://www.geeksforgeeks.org/problems/chocolate-distribution-problem/1) | Easy ✅ | Arrays, Greedy, Sliding Window | [View](./Week1/Day02_Chocolate_Distribution.java) |
| 3 | Feb 15 | [Meeting Rooms](https://www.geeksforgeeks.org/problems/meeting-rooms/1) | Easy ✅ | Arrays, Sorting, Intervals | [View](./Week1/Day03_Meeting_Rooms.java) |
| 4 | Feb 16 | [Maximum Overlapping Intervals](https://www.geeksforgeeks.org/problems/maximum-number-of-overlapping-intervals/1) | Hard 🔥 | Arrays, Sorting, Two Pointer, Sweep Line | [View](./Week1/Day04_Maximum_Overlapping_Intervals.java) |
| 5 | Feb 17 | [Count Inversions](https://www.geeksforgeeks.org/problems/inversion-of-array/1) | Medium 🟡 | Arrays, Merge Sort, Divide and Conquer | [View](./Week1/Day05_Count_Inversions.java) |
| 6 | Feb 18 | - | - | - | - |
| 7 | Feb 19 | - | - | - | - |

---

## 🏆 Milestones

- ✅ **Day 1:** Started the challenge!
- ✅ **Day 2:** Streak maintained!
- ✅ **Day 3:** Three-day streak! 🔥
- ✅ **Day 4:** Four-day streak! Solved 2nd Hard! 💪
- ✅ **Day 5:** Five-day streak! First Medium! 🎯
- ⏳ **Day 7:** First week complete
- ⏳ **Day 30:** Halfway there
- ⏳ **Day 60:** Challenge complete!

---

## 📚 Key Learnings

### Patterns Encountered:
- **Binary Search on Answer:** Day 1 (Painter's Partition)
- **Greedy + Sliding Window:** Day 2 (Chocolate Distribution)
- **Interval Scheduling:** Day 3 (Meeting Rooms)
- **Two Pointer + Sweep Line:** Day 4 (Maximum Overlapping Intervals)
- **Modified Merge Sort:** Day 5 (Count Inversions)
- More to come...

### Topics Covered:
- [x] Arrays
- [ ] Strings
- [x] Binary Search
- [ ] Dynamic Programming
- [ ] Graphs
- [ ] Trees
- [x] Greedy
- [ ] Recursion/Backtracking
- [x] Sorting
- [x] Intervals

---

## 💡 Problem Solving Approach

Each solution follows this structure:

1. **Problem Understanding:** What is being asked?
2. **Approach:** High-level strategy
3. **Algorithm:** Step-by-step logic
4. **Code:** Clean, commented implementation
5. **Complexity Analysis:** Time & Space complexity
6. **Key Takeaways:** What I learned

---

## 🔗 Connect With Me

- **LinkedIn:** [Niraj Jain](https://www.linkedin.com/in/nirajjain819/)
- **Codolio:** [nirajjain819](https://codolio.com/profile/nirajjain819)
- **GeeksforGeeks:** [nirajjain819](https://www.geeksforgeeks.org/user/nirajjain819/)

---

## 🎓 Parallel Journey

This challenge runs alongside my **Striver's A2Z DSA Sheet** journey. 

Following both:
- GFG POTD for consistency building
- Striver's Sheet for systematic learning

---

## 📝 How to Use This Repository

1. Browse solutions by week/day
2. Check `Solutions_Index.md` for quick navigation
3. Each solution file contains:
   - Problem link
   - Detailed explanation
   - Code with comments
   - Complexity analysis

---

## ⭐ Acknowledgments

- **GeeksforGeeks** for organizing this challenge
- **NPCI** for sponsoring the initiative
- The coding community for motivation and support

---

## 📄 License

This repository is for educational purposes. All problems are from GeeksforGeeks.

---

## 🔥 Let's Stay Accountable!

If you're also doing this challenge, let's connect! Drop a star ⭐ if you find this helpful.

**Current Streak:** 5 🔥  
**Target:** 60 🎯  
**Status:** Crushing it! 💪

---

*Last Updated: Feb 17, 2026*
